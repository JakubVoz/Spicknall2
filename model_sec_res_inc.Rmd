...
pepsi <- seq(0,0.5,by=0.025) #variation of epsilon
...
legend("bottomleft" , inset=.05, c("AMU=0.50", "AMU=0"), lty=c(1,1), lwd=c(2,2), col=c("red", "blue"))
...
legend("topleft" , inset=.05, c("AMU=0.50", "AMU=0"), lty=c(1,1), lwd=c(2,2), col=c("red", "blue"))
...
plot(t2, Newsec[,11], main="Incidence of secondary resistance \n after a change in AMU", xlab="time (steps)", ylab="new secondary Iz", ylim=c(0,0.01), lty=1, lwd=2, type="l")

capture.output(apply(Newsec, 2, linfun, t2), file='NUL')

legend("topright" , inset=.05, c("AMU=0.50", "AMU=0"), lty=c(1,1), lwd=c(2,2), col=c("red", "blue"))
...
par(mfrow=c(1,1))
plot(epsvar, p_ss, xlab="AMU", ylab="p", main = "Impact of AMU on steady state p", type="l", ylim=c(0,1))
```
...
plot(t2, NewIw_to_S[,11], main="Flow : clearance of sensitive \n cases after a change in AMU", xlab="time (steps)", ylab="Iw to S flow", ylim=c(0,0.02), lty=1, lwd=2, type="l")
