---
title: "inertie_study_time_to_equil_start_no_tr_cost"
author: "Jakub"
date: "13 ledna 2017"
...
delta_eps <- 0.01
pepsi_start <- seq(0.01,0.14,by=delta_eps) #initial values of epsilon
...
table_ss <- cbind(steady_S_eps, steady_Iw_eps, steady_Iz_eps, seq(0.01, 0.14, by=delta_eps))
```


```{r initial_ss_plot, echo=FALSE, cache=TRUE}
plot(pepsi_start, steady_Iz_eps, main="Starting equilibria \n no transmission cost", xlab="AMU", ylab="proportion", ylim=c(0,1), lty=2, lwd=2, type="l", col='red')
...
pepsi_change <- seq(0.01, 0.14, by=delta_eps)

plot(pepsi_change, time_up, main="Inertia - delta epsilon = 0.01 \n no transmission cost", xlab=expression(epsilon), ylab="time to stabilization", lwd=2, col='red', type='l', xlim=c(0,0.14))
...
plot(pepsi_change, time_up, main="Inertia \n no transmission cost", xlab=expression(epsilon), ylab="time to stabilization", lwd=2, col='red', type='l', xlim=c(0,0.14))
lines(pepsi_change, time_down, lwd=2, col='blue', type='l')
...
epsx1 <- seq(0.01, 0.14, by=0.01)


diff_2 <- time_up2 - time_down2
epsx2 <- seq(0.01, 0.14, by=0.001)


plot(epsx1, diff_1, main="Inertia : difference between decreasing \n and increasing epsilon \n no transmission cost", xlab=expression(epsilon), ylab="delta t", lwd=2, col='red', type='l', xlim=c(0,0.14))
...
epsx1 <- seq(0.01, 0.14, by=delta_eps1)


diff_2_same_metrics <- diff_2/delta_eps2
epsx2 <- seq(0.01, 0.14, by=delta_eps2)


plot(epsx2, diff_2_same_metrics, main="Inertia : comparison of differences between decreasing \n and increasing epsilon \n no transmission cost", xlab=expression(epsilon), ylab="delta t/ delta eps", lwd=2, col='blue', type='l', xlim=c(0,0.14))
