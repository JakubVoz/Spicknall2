...
```{r include=FALSE}install.packages("devtools")
library(deSolve)
N=10000
```
...
parameters <- c(beta_w=0.0001, beta_z=0.00005, gamma=0.005, gamma_prim=0.5, eps=0.7 )

state <- c(S1=N-1, S2=N-1, Iw=1, Iz=1)

times=seq(0, 30, by=1/12)
...
```{r single_strain_trial, echo=FALSE, fig.align='center', cache=TRUE}
plot(times, (out$S1 + out$Iw)/N, main="Plot n?1 - Single-strain model", xlab="time (yrs)", ylab="ratio", ylim=c(0,1), lty=1, lwd=2)
lines(times, out$Iw/N, lty=3, lwd=2)
lines(times, out$Iz/N, lty=4, lwd=2)
...
__Plot n?1 Single-strain model__: $\beta$ ~w~ = 0.0001, $\beta$ ~z~ = 0.00005, $\gamma$ = 0.005, $\gamma$?= 0.5 and $\epsilon$ = 0.7  are the chosen values of parameters. The $\delta$t = 1/12 year i.e. 1 month and we follow the evolution of the whole population (_solid line_), population infected with sensitive strain (_dotted line_) and with resistant strain (_dashed-dotted line_) over 30 years.
...
parameters <- c(beta_w=0.0001, beta_z=0.00005, gamma=0.005, gamma_prim=0.5, eps=0.7, q=0.25 )
...
```{r superinfection_trial, echo=FALSE, fig.align='center', cache=TRUE}
plot(times, (out_super$S + out_super$Iw + out_super$Iz + out_super$Iwz)/N, main="Plot n?2 - Superinfection model", xlab="time (yrs)", ylab="ratio", ylim=c(0,1), lty=1, lwd=2)
lines(times, out_super$S/N, lty=2, lwd=2)
lines(times, out_super$Iw/N, lty=3, lwd=2)
lines(times, out_super$Iz/N, lty=4, lwd=2)
lines(times, out_super$Iwz/N, lty=5, lwd=2)


```

__Plot n?2 Superinfection model__: $\beta$ ~w~ = 0.0001, $\beta$ ~z~ = 0.00005, $\gamma$ = 0.005, $\gamma$?= 0.5 and $\epsilon$ = 0.7 and $q$ = 0.25  are the chosen values of parameters. The $\delta$t = 1/12 year i.e. 1 month and we follow the evolution of the whole population (_solid line_), the sensitive subpopulation (_dashed line_), the subpopulation infected with sensitive strain exclusively (_dotted line_), with resistant strain exclusively (_dashed-dotted line_) and with both (_long-dashed line_) over 30 years.
...
```{r exclusive_trial, echo=FALSE, fig.align='center', cache=TRUE}
plot(times, (out_exc$S + out_exc$Iw + out_exc$Iz)/N, main="Plot n?3 - Exclusive infection model", xlab="time (yrs)", ylab="ratio", ylim=c(0,1), lty=1, lwd=2)
lines(times, out_exc$S/N, lty=2, lwd=2)
lines(times, out_exc$Iw/N, lty=3, lwd=2)
lines(times, out_exc$Iz/N, lty=4, lwd=2)

```



__Plot n?3 Exclusive infection model__: $\beta$ ~w~ = 0.0001, $\beta$ ~z~ = 0.00005, $\gamma$ = 0.005, $\gamma$?= 0.5, $\epsilon$ = 0.7 and $q$=0.25  are the chosen values of parameters. The $\delta$t = 1/12 year i.e. 1 month and we follow the evolution of the whole population (_solid line_), the sensitive subpopulation (_dashed line_), the subpopulation infected with sensitive strain exclusively (_dotted line_) and with resistant strain exclusively (_dashed-dotted line_) over 30 years.
...
```{r replacement_trial, echo=FALSE, fig.align='center', cache=TRUE}
plot(times, (out_rep$S + out_rep$Iw + out_rep$Iz)/N, main="Plot n?4 - Replacement infection model", xlab="time (yrs)", ylab="ratio", ylim=c(0,1), lty=1, lwd=2)
lines(times, out_rep$S/N, lty=2, lwd=2)
lines(times, out_rep$Iw/N, lty=3, lwd=2)
lines(times, out_rep$Iz/N, lty=4, lwd=2)

```



__Plot n?4 Replacement infection model__: $\beta$ ~w~ = 0.0001, $\beta$ ~z~ = 0.00005, $\gamma$ = 0.005, $\gamma$?= 0.5 and $\epsilon$ = 0.7  are the chosen values of parameters. The $\delta$t = 1/12 year i.e. 1 month and we follow its evolution during 30 years. The $\delta$t = 1/12 year i.e. 1 month and we follow the evolution of the whole population (_solid line_), the sensitive subpopulation (_dashed line_), the subpopulation infected with sensitive strain exclusively (_dotted line_) and with resistant strain exclusively (_dashed-dotted line_) over 30 years.
...
parameters <- c(beta_w=0.0001, beta_z=0.00005, gamma=0.005, gamma_prim=0.5, eps=0.7, rho=0.01 )
...
```{r unidir_trial, echo=FALSE, fig.align='center', cache=TRUE}
plot(times, (out_uni$S + out_uni$Iw + out_uni$Iz)/N, main="Plot n?5 - Unidirectional conversion model", xlab="time (yrs)", ylab="ratio", ylim=c(0,1), lty=1, lwd=2)
lines(times, out_uni$S/N, lty=2, lwd=2)
lines(times, out_uni$Iw/N, lty=3, lwd=2)
lines(times, out_uni$Iz/N, lty=4, lwd=2)

```



__Plot n?5 Unidirectional conversion model__: $\beta$ ~w~ = 0.0001, $\beta$ ~z~ = 0.00005, $\gamma$ = 0.005, $\gamma$?= 0.5, $\epsilon$ = 0.7 and $\rho$ = 0.01  are the chosen values of parameters. The $\delta$t = 1/12 year i.e. 1 month and we follow the evolution of the whole population (_solid line_), the sensitive subpopulation (_dashed line_), the subpopulation infected with sensitive strain exclusively (_dotted line_) and with resistant strain exclusively (_dashed-dotted line_) over 30 years.
...
parameters <- c(beta_w=0.0001, beta_z=0.00005, gamma=0.005, gamma_prim=0.5, eps=0.7, rho=0.01 , phi=0.02)
...
```{r bidir_trial, echo=FALSE, fig.align='center', cache=TRUE}
plot(times, (out_bi$S + out_bi$Iw + out_bi$Iz)/N, main="Plot n?6", xlab="time (yrs)", ylab="ratio", ylim=c(0,1), lty=1, lwd=2)
lines(times, out_bi$S/N, lty=2, lwd=2)
lines(times, out_bi$Iw/N, lty=3, lwd=2)
lines(times, out_bi$Iz/N, lty=4, lwd=2)

```



__Plot n?6 Bidirectional conversion model__: $\beta$ ~w~ = 0.0001, $\beta$ ~z~ = 0.00005, $\gamma$ = 0.005, $\gamma$?= 0.5, $\epsilon$ = 0.7, $\rho$ = 0.01 and $\phi$=0.02 are the chosen values of parameters. The $\delta$t = 1/12 year i.e. 1 month and we follow the evolution of the whole population (_solid line_), the sensitive subpopulation (_dashed line_), the subpopulation infected with sensitive strain exclusively (_dotted line_) and with resistant strain exclusively (_dashed-dotted line_) over 30 years.
