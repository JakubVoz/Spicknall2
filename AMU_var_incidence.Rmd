...
```{r QA_as_func_t_and_vareps, echo=FALSE, fig.align='center'}
...
for (i in 1:length(pepsi)) {
  QA_Iw[,i] <- QA_Iw[,i]*pepsi[i]
}

QA_Iw_Iz <- Iw + Iz

for (i in 1:length(pepsi)) {
  QA_Iw_Iz[,i] <- QA_Iw_Iz[,i]*pepsi[i]
}
...
QA_Iw_zoom <- vrai_Iw_zoom

for (i in 1:length(pepsi)) {
  QA_Iw_zoom[,i] <- QA_Iw_zoom[,i]*pepsi[i]
}

QA_Iw_Iz_zoom <- vrai_Iw_zoom + vrai_Iz_zoom

for (i in 1:length(pepsi)) {
  QA_Iw_Iz_zoom[,i] <- QA_Iw_Iz_zoom[,i]*pepsi[i]
}



plot(t2, QA_Iw[,11], main="QA in time, Iw treated", xlab="time (steps)", ylab="QA", ylim=c(0,0.1), lty=1, lwd=2, type="l")

capture.output(apply(QA_Iw, 2, linfun, t2), file='NUL')

legend("topleft" , inset=.05, c("PTP=0.20", "PTP=0"), lty=c(1,1), lwd=c(2,2), col=c("red", "blue"))


plot(t2, QA_Iw_Iz[,11], main="QA in time, all treated", xlab="time (steps)", ylab="QA", ylim=c(0,0.1), lty=1, lwd=2, type="l")

capture.output(apply(QA_Iw_Iz, 2, linfun, t2), file='NUL')
...
#zoom plot
plot(time_zoom, QA_Iw_zoom[,11], main="QA in time, Iw treated", xlab="time (steps)", ylab="QA", ylim=c(0,0.1), lty=1, lwd=2, type="l")

capture.output(apply(QA_Iw_zoom, 2, linfun, time_zoom), file='NUL')

legend("topleft" , inset=.05, c("PTP=0.20", "PTP=0"), lty=c(1,1), lwd=c(2,2), col=c("red", "blue"))


plot(time_zoom, QA_Iw_Iz_zoom[,11], main="QA in time, all treated", xlab="time (steps)", ylab="QA", ylim=c(0,0.1), lty=1, lwd=2, type="l")

capture.output(apply(QA_Iw_Iz_zoom, 2, linfun, time_zoom), file='NUL')
...
```{r p_incid_as_func_QA_ss, echo=FALSE, fig.align='center'}

QA_var_ss <- burden*epsvar

QA_var_ss <- data.matrix(QA_var_ss)
realp_ss <- data.matrix(realp_ss)

plot(QA_var_ss, realp_ss, main="P incidence, all treated", xlab="QA", ylab="real p", ylim=c(0,1), lty=1, lwd=2, type="l")
