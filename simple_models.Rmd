...
plot(t2, NewIw[,11], main="Incidence of susceptible infections \n after a change in AMU", xlab="time (steps)", ylab="Incidence Iw", ylim=c(0.007,0.01), lty=1, lwd=2, type="l")
...
```{r Iw_S_ratio_as_fn_Iw, echo=FALSE, cache=TRUE}
Iw_pos <- seq(0, 1, by=0.01)

S_pos <- 1-Iw_pos

Iw_pos <- as.matrix(Iw_pos)
S_pos <- as.matrix(S_pos)

Iw_S_product <- S_pos*Iw_pos

Iw_S_product <- as.matrix(Iw_S_product)

plot(Iw_pos, Iw_S_product, xlab="Iw ratio", ylab="Iw S mass product", main = "Iw S mass product", lwd=2, col='blue', ylim=c(0,1))

plot(S_ss, Iw_S_product, xlab="S ratio", ylab="Iw S mass product", main = "Iw S mass product", type="l", lwd=2, col='blue', ylim=c(0,1))
