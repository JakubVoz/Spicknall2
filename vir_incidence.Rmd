...
state <- c(S=1-2*prop, Iw=prop, Iz=prop, dnewcasesIz=0, dnewcasesIw=0, dsecondaryres=0, dprimaryres=0, newS_to_Iw=0, Iz_to_Iw=0, Iw_to_S=0, Iw_to_Iz=0, newS_to_Iz=0, Iz_to_S=0, virulence=0)
...
SIS=function(times, state, parameters){
  with(as.list(c(state,parameters)),
       { dnewcasesIz=beta_z*Iz*S + eps*beta_z*Iz*Iw + rho*eps*Iw
        ...
        dvirulence = alpha*(Iw+Iz)
        
        
        
        list(c(dS, dIw, dIz, dnewcasesIz, dnewcasesIw, dsecondaryres, dprimaryres, dnewS_to_Iw, dIz_to_Iw, dIw_to_S,  dIw_to_Iz, dnewS_to_Iz, dIz_to_S, dvirulence))
       })
}
...
parameters3 <- c(beta_w=pbeta_w, beta_z=pbeta_z, gamma=pgammaa, gamma_prim=pgamma_tilt, eps=peps, rho=prho, alpha=palpha)


times3=seq(a, a+zoom_area, by=m/zoom)

state3 <- c(S=out_init$S[a+1], Iw=out_init$Iw[a+1], Iz=out_init$Iz[a+1], Iwz=out_init$Iwz[a+1], dnewcasesIz=0, dnewcasesIw=0, dsecondaryres=0, dprimaryres=0, newS_to_Iw=0, Iz_to_Iw=0, Iw_to_S=0, Iw_to_Iz=0, newS_to_Iz=0, Iz_to_S=0, virulence=0)
...
nnewiz <- seq(5,ncol(out), by=15)
nnewiw <- seq(6,ncol(out), by=15)
nnewsec <- seq(7,ncol(out), by=15)
nnewprim <- seq(8,ncol(out), by=15)

nS_to_Iw <- seq(9,ncol(out), by=15)
nS_to_Iz <- seq(13,ncol(out), by=15)
nIz_to_Iw <- seq(10,ncol(out), by=15)
nIw_to_S <- seq(11,ncol(out), by=15)

nIw_to_Iz <- seq(12,ncol(out), by=15)
nIz_to_S <- seq(14,ncol(out), by=15)

nvir <- seq(15,ncol(out), by=15)
...
NewVirulence <- apply(NewVirulence, 2, incid, zoom)
...
virulence_zoom <- NewVirulence[1:(zooming+1),]
...
nS <- seq(2,ncol(out), by=15)
nIw <- seq(3,ncol(out), by=15)
nIz <- seq(4,ncol(out), by=15)
...
p_ss <- Newp[nrow(Newp),]
viru_ss <- NewVirulence[nrow(NewVirulence),]
...
```{r viru_plot, echo=FALSE, fig.align='center', cache=TRUE}

par(mfrow=c(1,2))

 
plot(t2, NewVirulence[,11], main="Virulence evolution \n after a change in epsilon", xlab="time (steps)", ylab="Virulence", ylim=c(0,0.001), lty=1, lwd=2, type="l")

capture.output(apply(NewVirulence, 2, linfun, t2),file='NUL') 

legend("bottomleft" , inset=.05, c("eps=0.20", "eps=0"), lty=c(1,1), lwd=c(2,2), col=c("red", "blue"))

#zoom

plot(time_zoom, virulence_zoom[,11], main="Dynamics", xlab="time (steps)", ylab="Virulence", ylim=c(0,0.001), lty=1, lwd=2, type="l")

capture.output(apply(virulence_zoom, 2, linfun, time_zoom), file='NUL')
...
par(mfrow=c(1,1))
plot(epsvar, viru_ss, xlab=expression(epsilon), ylab="virulence", main = "Impact of epsilon on steady state Virulence", ylim=c(0, 0.0008), type="l")
```
...
plot(t2, Realp[,11], main="Estimated real p after a change in epsilon", xlab="time (steps)", ylab="Real p", ylim=c(0,1), lty=1, lwd=2, type="l")
