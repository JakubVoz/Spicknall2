...
\begin{aligned}
\beta_w &= \beta_{w_0} - a*QA\\
\beta_z &= \beta_{z_0} + b*QA\\
\rho' &= c*QA\\
\end{aligned}

There are different scenarios how to grasp QA depending on the efficiency of AMR detection :

QA expression | Scenario
------------------|-------------------------------------
$QA = I_w*\varepsilon$ | Ideal detection
$QA = (I_w+I_z)*\varepsilon$ | No detection
$QA = I_w*\varepsilon + ENV$ | Environmental use + Ideal detection
$QA = (I_w+I_z)*\varepsilon + ENV$ | Environmental use + No detection
...
SIS_no_detection=function(times, state, parameters){
  with(as.list(c(state,parameters)),
       { 
         Q = eps*(Iw+Iz)
         beta_w = beta_w_basic - Q*rate_beta_w
         beta_z = beta_z_basic + Q*rate_beta_z
         rho_prim = Q*rate_rho_prim
...
``` {r steady_state_eps, include=FALSE}
